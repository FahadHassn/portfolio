    ...
    <section id="services" className="py-16 lg:py-24 relative">
      ...
      <div className="container-x">
        ...
        <div className="mt-12 lg:mt-16 flex flex-nowrap items-stretch overflow-x-auto hide-scrollbar md:grid md:grid-cols-2 md:overflow-visible lg:grid-cols-3 gap-5 pb-8 md:pb-0 snap-x snap-mandatory px-4 md:px-0 -mx-4 md:mx-0 py-4 -my-4 md:py-0 md:my-0">
          {services.map((s, i) => (
            <article key={s.title} className="flex flex-col self-stretch w-[75vw] sm:w-[60vw] md:w-full min-w-0 shrink-0 snap-center md:snap-align-none card-hover group rounded-2xl border border-border bg-elevated p-8 relative overflow-hidden transition-all hover:shadow-[0_4px_30px_rgba(200,168,75,0.06)] outline-none focus:outline-none items-center text-center md:items-start md:text-left">
              {/* Gold corner accent on hover */}
                ...
                <div className="flex flex-col md:flex-row items-center gap-4 mt-6">
                  <div className={`inline-flex shrink-0 h-14 w-14 items-center justify-center rounded-xl transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(200,168,75,0.2)] ${
                    s.tone === "gold" ? "bg-primary/10 text-primary border border-primary/20" : "bg-surface text-muted-foreground border border-border group-hover:text-primary group-hover:border-primary/20"
                  }`}>
